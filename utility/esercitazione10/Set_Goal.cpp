...
void SetGoal_CallBack(const set_goal::NewGoal& new_goal);
{
	...
	new_goal_msg.frame_id="map";
	
	new_goal_msg.position.x=new_goal.x;
	new_goal_msg.position.y=new_goal.y;
	new_goal_msg.position.z=0;
...
void position_CallBack(const ft2_msgs)
{
	int transform_ok=tBuffer.canTransform("map","base_link",ros::Time(0));
	if(transform_ok!=0)//posso transformare 
	{
		geometry_msgs::TransformStamped transformStamped;
		transformStamped = tBuffer.lookupTransform("map","base_link",ros::Time(0));
...
int main(int argc,char **argv)
{
	...
	ros::Subsctiber sub_tf=n.subscribe("tf",1000,position_CallBack);
	...
	while(ros::ok())
	{
		...
		{
			ROS::INFO("Publishing a new goal position");
